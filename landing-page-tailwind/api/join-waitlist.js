...
const resend = new Resend(process.env.RESEND_API_KEY || 're_QdhrAVfE_DXYtuS8vkhdwR865GuoQE2Tt');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (error && error.code === '23505') {
      ...
      // Send a welcome email anyway, in case they lost the first one
      await sendWelcomeEmail(email, signedUrlData.signedUrl);
      
      return res.status(200).json({ 
        message: 'You\'re already on our waitlist! We\'ve sent the instructions again.' 
      });
    }
    ...
    // Send welcome email with early access instructions and download link
    await sendWelcomeEmail(email, signedUrlData.signedUrl);

    // Return success response
    return res.status(200).json({ 
      message: 'Successfully joined the waitlist! Check your email for instructions.' 
    });
  } catch (error) {
...
async function sendWelcomeEmail(email, downloadLink) {
  try {
      ...
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #1e1e1e; max-width: 600px; margin: auto; padding: 24px;">
          ...
          <p><strong>🔓 Download your extension:</strong><br>
            <a href="${downloadLink}" style="display: inline-block; margin: 12px 0; padding: 12px 20px; background-color: #3b82f6; color: #fff; text-decoration: none; border-radius: 6px;">Download Mindtrail</a>
          </p>
